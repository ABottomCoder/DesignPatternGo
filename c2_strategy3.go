...
func main() {
	...
	switch method {
	case "normal":
		cc = CashContext{cs: CashNormal{}}
	case "rebate":
		println("Please enter a rebate number: ")
		var rebate float64
		fmt.Scan(&rebate)
		cc = CashContext{CashRebate{moneyRebate: rebate}}
	case "return":
		println("Please enter condition and myReturn number: ")
		var condition, myReturn float64
		fmt.Scanln(&condition, &myReturn)
		cc = CashContext{CashReturn{moneyCondition: condition, moneyReturn: myReturn}}
	}

	charge := cc.GetResult(total)
