...
func (c CashFactory) GetFactory(s string) CashSuper {
	switch s {
	case "normal":
		return CashNormal{}
	case "rebate":
	...
	case "return":
		...
		return CashReturn{moneyCondition: condition,
			moneyReturn: myReturn}
	}
	return nil
}
...
func main() {
	...
	fmt.Scan(&method)
	factory := CashFactory{}
	cash := factory.GetFactory(method)
	charge := cash.AcceptCash(total)
